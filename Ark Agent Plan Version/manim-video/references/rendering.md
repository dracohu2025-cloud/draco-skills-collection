...
## Fit Narration to an Existing Video Length

When you already have a rendered video and the narration is a few seconds too short or too long, do **not** immediately rewrite the script or re-render everything. First measure both durations and apply a small tempo correction.

```bash
# Measure durations
VIDEO_DUR=$(ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 final.mp4)
AUDIO_DUR=$(ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 narration.mp3)

# Compute tempo ratio = audio_duration / video_duration
python - << 'PY'
a = float("${AUDIO_DUR}")
v = float("${VIDEO_DUR}")
print(a / v)
...
# Example: audio is 33.096s, video is 37.0s -> atempo=0.8944864865
ffmpeg -y -i narration.mp3 -filter:a "atempo=0.8944864865" narration_fit.mp3

# Then mux the fitted narration
ffmpeg -y -i final.mp4 -i narration_fit.mp3 \
  -c:v copy -c:a aac -b:a 192k -shortest final_narrated.mp4
```

Guideline: keep `atempo` adjustments modest (roughly within 0.9-1.1 when possible). A small correction is usually less painful than re-recording or re-timing the whole animation.
