...
- **汤底 + 佐料风格库**，固定一套高质感封面底层逻辑，再从 10 组可复用视觉佐料中自动选型
- **只改关键巧思**，每篇文章只变 1-3 个标题/主题相关元素，避免每次都随机发散
...
flowchart LR
  ...
  C --> D[选择固定汤底 + 10组佐料之一]
  D --> E[生成 review_spec 和瘦身 execution_spec]
  E --> F[Nano Banana / Gemini Flash Image 出图]
  F --> G[本地封面图文件]
  G --> H[可选：上传飞书云盘]
  G --> I[可选：上传微信公众号封面素材]
```
...
- **汤底**：先判断文章为什么值得被看见，把复杂信息收束为一个手机端 3 秒可读的封面理由。
- **佐料**：从 10 组风格中选一种，例如建筑图纸、科技莫兰迪、东方文人、电影片头、产品手册、艺术展览等。
- **巧思**：只根据文章标题/主题改 1-3 个元素，保持系列感，也保留新鲜感。

导出的 JSON 里会同时有：
- `review_spec`：给人审。
- `execution_spec`：真正发给图片模型，字段更少，画面更稳。
