...
一个专门做 **AI / Agent / AIGC 每日早报** 的 Hermes skill，**钉钉专用版**。

它适合每天固定时间自动跑：抓取过去 24 小时的重要更新，核验来源，写成中文早报，发布到钉钉文档，并可同步归档到钉钉多维表。
...
| 钉钉发布 | 输出为钉钉原生文档 |
| 钉钉多维表归档 | 可写入钉钉多维表，保留标题、Top3、摘要、链接等字段 |
...
- Python 3
- `git`
- `dws` CLI（钉钉命令行工具），并完成钉钉登录授权
- [`news-aggregator-skill`](../news-aggregator-skill/)：抓取新闻候选
- [`ai-news-bitable-archive-dingtalk`](../ai-news-bitable-archive-dingtalk/)：归档日报到钉钉多维表

如果要完整跑钉钉发布和多维表归档，需要准备：

```text
DINGTALK_BASE_ID=<你的钉钉多维表 base id>
DINGTALK_TABLE_ID=<你的钉钉多维表 table id>
DINGTALK_FOLDER_ID=<你的钉钉文档文件夹 id（可选）>
```
...
必须使用 daily-ai-agent-aigc-top-news-dingtalk skill。
...
5. 发布为钉钉文档。
6. 创建后必须 dws doc read 回读验收。
7. 如启用归档，必须按 record_id 回读验收。
8. 最终回复包含：文档标题、doc_url、aitable_url、record_id、3～6条摘要。
```
...
| [`references/dingtalk-adaptation.md`](./references/dingtalk-adaptation.md) | 钉钉适配方案参考 |
