# Archived skill: feishu-doc-to-wechat-draft

Original path: `productivity/feishu-doc-to-wechat-draft`

Consolidation reason: Feishu-to-WeChat draft is a source-to-publish route in the WeChat workflow.
...
现在这条链路对飞书文档中的视频块（典型形态：`<view type="2"><file token="..." name="xxx.mp4"/></view>`）新增了**视频同步降级方案**：

1. 识别飞书文档中的 `.mp4` 视频块
2. 发布到公众号时，先用 `lark-cli docs +media-download` 下载视频文件
3. 调微信永久素材接口：
   - `material/add_material?type=video`
   - 上传成公众号视频素材，得到 `media_id`
4. 用 `ffmpeg` 抽取视频首帧封面
5. 再将封面图上传为图文正文图片 URL
6. 在公众号正文中渲染成一个**视频卡片占位**：
   - 显示封面图
   - 显示视频文件名
   - 明确提示“已同步到公众号视频素材库；公众号正文暂不支持内嵌播放”
...
真实能力边界是：
- **公众号支持上传视频素材到素材库**
- 但 **`draft_add` 的正文 `content` 不支持直接用 `media_id` 内嵌视频播放**
...
"video_materials": [
  {
    "token": "...",
    "name": "seedance-rollercoaster-11s-480p.mp4",
    "media_id": "...",
    "cover_url": "...",
    "source_url": "..."
  }
...
则：
- 不让整篇文章发布直接失败
- 正文中保留一个“视频同步失败”的占位卡片
- 继续发布其余正文内容

### 与真实文档联调的经验

这次已真实验证：
- 飞书视频 token 可通过 `lark-cli docs +media-download` 成功下载
- 单个 `.mp4` 可成功上传到微信永久视频素材库
- 可成功抽首帧并上传成正文图片
- 最终草稿正文里可看到视频封面卡片 + 提示语

一句话记忆：**飞书视频块现在可以“同步到公众号草稿箱”，但形式是“视频素材入库 + 正文封面卡片占位”，不是正文内直接播放。**
...
### 新增经验：默认封面优先使用文章第一张图片

`publish-feishu-doc-default` 已调整：正式发布时如果没有显式传入：
- `--thumb-media-id`
- `--cover-image`

则默认提取文章正文里的第一张图片作为公众号封面，并上传成 `thumb_media_id`。

优先级固定为：
1. `--thumb-media-id`
2. `--cover-image`
3. 正文第一张图片（支持飞书 `lark-image://...`，也支持本地/远程图片）

这样飞书文档直推公众号草稿箱时，不再默认新生成临时封面。只有文章没有任何图片时，才需要额外提供封面。

发布后仍要回查：
- `draft/get` 中 `thumb_media_id` 非空
- 正文中 `lark-image://` 数量为 0
- `draft/batchget` 能列出本次草稿

### 历史做法：没有现成 `thumb_media_id` 时，才本地做一个临时封面

早期把新的飞书文档直推公众号草稿箱时，真实踩到的第一道坎仍然是：
...
也就是说：
- 现在只有当文章没有图片、也没有传封面时，才需要额外提供：
  - `--thumb-media-id`
  - `--cover-image`

如果手头没有现成封面素材，且文章本身没有图片，才用临时封面：
...
5. 再用 `draft/get` 拉回正文内容做最小内容质量核验：
   - `img_tags` 数量合理
   - `lark-image://` 已全部替换为微信图片 URL，数量应为 0
   - 不应出现旧 code block 回归特征：`white-space: nowrap`、`display: -webkit-box`
   - 应保留横滑特征：`overflow-x: auto`、`-webkit-overflow-scrolling: touch`

如果用户反馈“草稿箱里没看到新稿”，优先不要争论；直接重新做这一步回查。

### 新增经验：快速推送时也要做“预览 → dry-run → 正式发布 → 回查内容”四段核验

这次把飞书文档 `Hermes如何接入GPT-Image-2` 推到公众号草稿箱时，最稳的最小流程是：

1. `render-preview-feishu-doc-default` 生成 HTML，先读出标题、图片引用数量，并检查 code block 关键样式
2. `publish-feishu-doc-default --dry-run --thumb-media-id DRY_RUN_MEDIA_ID` 组装 payload，确认标题、作者、摘要长度、正文大小
3. 如果没有现成 `thumb_media_id`，用 `ffmpeg` 生成临时 900x383 封面 PNG，再用 `--cover-image` 正式发布
4. 发布后不要只信 CLI 返回的 `draft_media_id`：
   - `draft/get` 核对标题、作者、封面、正文长度
   - `draft/batchget` 确认当前公众号草稿列表能看到这条记录
   - 再从 `draft/get` 的正文里检查图片替换和 code block 样式标记

注意：dry-run 阶段可能只验证 payload 结构，不代表飞书图片已经实际上传并替换；图片是否已从 `lark-image://` 变成微信 URL，要以正式发布后的 `draft/get` 正文为准。
...
- 但你持续迭代、测试、修 bug 的 canonical 项目代码，可能在：
  - `<repo>/feishu-doc-to-wechat-draft/`
...
CANONICAL_ROOT = Path("<repo>/feishu-doc-to-wechat-draft/scripts")
...
diff -u \
  ...
  <repo>/feishu-doc-to-wechat-draft/scripts/wechat_draft_publisher/renderer.py
```
...
## 新增经验：整篇 HTML 做 XML 解析改写列表时，先把 void 标签变成 XML-safe

这次真实回归又踩到一个更隐蔽的坑：

- 无序列表改写逻辑虽然已经从“正则硬拆”升级成了树解析
- 但如果你直接对**整篇已渲染 HTML**执行 `ElementTree.fromstring(...)`
- 文中只要混入一个合法 HTML 但**不合法 XML** 的 void 标签，例如：
  - `<hr ...>`
  - `<br>`
- 整个解析就会失败
- 结果不是“某一个列表坏掉”，而是**整篇文章的无序列表重写全部被跳过**
- 外在表现就是：
  - 微信草稿里很多本该补出来的点状 `•` 全没了
  - 又退回成原始 `<ul class="md-ul"> ... </ul>`

这次的最小复现就是：
- 文章里前面有一个 `---`（会被渲染成 `<hr>`）
- 后面再出现无序列表
- 若 `_rewrite_unordered_lists()` 在进入 XML 解析前不先做 XML-safe 规范化，bullet rewrite 就会整体失效

稳妥做法：

1. 在把 HTML 送进 `ElementTree` 之前，先做一层 `XML-safe fragment` 归一化
2. 至少把这些标签改成自闭合：
   - `<hr ... />`
   - `<br />`
3. 然后再执行整篇树解析与列表重写
...
1. markdown 输入中同时包含：
   ...
   - 一个无序列表
2. 断言输出中：
   - 存在 `class="md-bullet-dot"`
   - 不再残留 `<ul class="md-ul"`

一句话记忆：**就算列表改写已经用树处理，只要你解析的是“整篇 HTML”，也得先把 `hr/br` 这类 void 标签处理成 XML-safe；否则一次解析失败，会让整篇文章的 bullet 全部失效。**