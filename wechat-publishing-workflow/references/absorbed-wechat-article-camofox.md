# Archived skill: wechat-article-camofox

Original path: `productivity/wechat-article-camofox`

Consolidation reason: Camofox extraction is one backend for WeChat article ingestion.
...
- 本机命令入口：`wechat-article-fetch`
- 本机命令入口：`wechat-article-to-feishu`
...
```bash
wechat-article-to-feishu "https://mp.weixin.qq.com/s/xxxxxxxxxxxxxxxx"
```
...
```bash
wechat-article-fetch "https://mp.weixin.qq.com/s/xxxxxxxxxxxxxxxx"
```
...
```bash
wechat-article-fetch "https://mp.weixin.qq.com/s/xxxxxxxxxxxxxxxx" --format json
```

### 3）保存到文件

```bash
wechat-article-fetch "https://mp.weixin.qq.com/s/xxxxxxxxxxxxxxxx" --save /tmp/article.md
```

### 4）附带图片列表

```bash
wechat-article-fetch "https://mp.weixin.qq.com/s/xxxxxxxxxxxxxxxx" --format json --include-images
```

## Python 脚本直接调用

```bash
python ~/.hermes/skills/productivity/wechat-article-camofox/scripts/fetch_wechat_article.py \
